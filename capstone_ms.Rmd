...
It is important to mention that sparse terms have been removed at earlier stage of documents processing. Black line shows **log(median)** which is around **`r round(log(median(freq.df.l$unigram$Freq)), 2)`** which corresponds to **median** = **`r round(median(freq.df.l$unigram$Freq), 2)`**.  
This means that after removing sparse terms, half of all the words in sample documents occur less than or equal to `r round(median(freq.df.l$unigram$Freq), 2)` times.

```{r displayHist1, echo = FALSE}

# terms frequencies histogram
qplot(log(Freq), data = freq.df.l$unigram, geom = "histogram", bins = 30, fill=..count..) +
    labs(x = "log(Term Frequency)",
         y = "Count",
         title = "Histogram of term frequencies") +
    geom_vline(xintercept = log(median(freq.df.l$unigram$Freq)), size = 2, color = "black")
```
